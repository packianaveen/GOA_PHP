...
$SV1 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$PilotFlameAirc, 1));
$SV2 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$FuelInjectorAirc, 1));
$SV3 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$PilotFlameGasc, 1));
$SV4 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$FCVAirc, 1));
$SV5 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$FCVKeroseneFuelc, 1));
$SV6 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$ByPassValueIc, 1));
$SV7 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$ByPassValueIIc, 1));
$SV8 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$IgnitorSwitchc, 1));
$SV9 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$KerosenePumpc, 1));
$SV10 = PhpType::bytes2signedInt($modbus->readMultipleRegisters(0,$LubeOilPumpc, 1));
$valves = [$SV1,$SV2,$SV3,$SV4,$SV5,$SV6,$SV7,$SV8,$SV9,$SV10];
...
if($nshutdowncompleted == 1){
	$inserttestData = mysqli_query($conn,"INSERT INTO `enertek_combuster_goa`.`testcommands`(`test_id`,`name`,`index`,`type`,`testcommandsTime`,`valvestatus`) VALUES('$test_id','Shutdown Initiated','S10','s',CURRENT_TIME(),'$valves')");
...
if($eshutdown == 1){
	$inserttestData = mysqli_query($conn,"INSERT INTO `enertek_combuster_goa`.`testcommands`(`test_id`,`name`,`index`,`type`) VALUES('$test_id','eshutdown','S11','s')");
	$count++;
	break;
}
